...
if (Yii::$app->user->identity->unit_id == 'DINKES'){
    $this->params['breadcrumbs'][] = ['label' => 'Data POA Puskesmas '.$session['deptPeriodValue'], 'url' => ['deptperiod/list', 'period' => $session['deptPeriodValue']]];
}else{
        ...
        ['class' => 'yii\grid\ActionColumn',
            ...
            'buttons' => [
                ...
                'delete' => function ($url, $model, $session) {
                    ...
                    //                 $.ajax({
                    //                     type: 'POST',
                    //                     url: window.origin +'".Url::to(['deptperfomance/delete', 'id' => $model['perfomance_id']])."',
                    //                     data: '',
                        ...
                        if(!empty($model['realization_id'])){
                            return Html::button('<span class="glyphicon glyphicon-trash"></span>', [
                                ...
                                'onclick' => "if (confirm('Apakah realisasi akan dihapus?')) {
                                    $.ajax({
                                        type: 'POST',
                                        url: window.origin +'".Url::to(['deptrealization/delete', 'id' => $model['realization_id']])."',
                                        data: '',
                                        success: function(result) {
                                            if(result == 0) {
                                                $.pjax.reload('#real', {timeout: false});

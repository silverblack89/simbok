...
$this->title = 'Detail per Sub Kegiatan DPA';
    ...
    <div class="row">
        <div class="col-sm-1">
            <?php if(Yii::$app->user->identity->username == 'admin'){ //$session['poaLabel'] == ' Awal' && ?> 
                <?= Html::a('<span class="glyphicon glyphicon-export"></span> Export', ['exportxlsdpa'], ['title' => 'Export Excel', 'class' => 'btn btn-default']) ?>
            <?php }else{ ?>
                <?= Html::a('<span class="glyphicon glyphicon-export"></span> Export', ['exportxlsdpa'], ['title' => 'Export Excel', 'class' => 'btn btn-default']) ?>
            <?php } ?>
        ...
        <div class="col-sm-11">
            <?= Html::dropDownList('komponen', null, ArrayHelper::map(Dpa::find()->where(['tahun' => $session['deptPeriodValue']])
                                                                    ->all(),'id','nama' ),
                                                    [
                                                        'options'=>[$session['dpa']=>['Selected'=>true]],

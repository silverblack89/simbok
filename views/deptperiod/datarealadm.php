...
if (Yii::$app->user->identity->group_id == 'ADM'){
    $this->title = 'Data Realisasi '.$namaUnit.' Triwulan '.$r;
    $visible = true;
    ...
    $this->title = 'Data Realisasi '.$namaUnit;
    $visible = false;
...
?>
    ...
    <?php if(Yii::$app->user->identity->group_id == 'SEK') {?>
        ...
        [
            ...
            'onchange'=>'
                $.pjax.reload({
                    url: "'.Url::to(['deptperiod/datarealadm']).'?id="+$(this).val(),
                    container: "#datareal",
                    timeout: 1000,
                });',
            'class'=>'form-control']) 
    ...
    <?php }else{ ?>
        ...
        [
            ...
            'onchange'=>'
                $.pjax.reload({
                    url: "'.Url::to(['deptperiod/datarealadm', 'unit_id' => $unit_id, 'p' => $p]).'&bok_id="+$(this).val(),
                    container: "#datareal",
                    timeout: 1000,
                });',
            'class'=>'form-control']) 
        ...
        'columns' => [
            ...
            'buttons' => [
                'user' => function ($url, $model) {
                    // return Html::a('Proses', array('deptdatareal/index', 'id'=>$model['id']), ['class'=>'btn btn-xs btn-info custom_button']);
                    return Html::button('<span class="glyphicon glyphicon-check"></span> Proses', ['value' => Url::to(['deptdatareal/index', 'id' => $model['id']]), 'title' => 'Realisasi', 'class' => 'showModalButton btn btn-xs btn-info custom_button']);
                },
            ]
        ],
